...
class _StylusStatusSection extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<_StylusStateBloc, StylusType>(
      builder: (context, stylusType) {
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const _SectionHeader('Detected Stylus'),
            ListTile(
              leading: Icon(
                Icons.draw_outlined,
                color: _colorForType(context, stylusType),
              ),
              title: Text(_labelForType(stylusType)),
              subtitle: Text(_subtitleForType(stylusType)),
              trailing: stylusType != StylusType.unknown &&
                      stylusType != StylusType.finger
                  ? const Icon(Icons.check_circle, color: Colors.green)
                  : const Icon(Icons.help_outline),
            ),
            if (stylusType != StylusType.unknown &&
                stylusType != StylusType.finger)
              _CapabilityChips(stylusType: stylusType),
          ],
        );
      },
    );
  ...
  Color _colorForType(BuildContext context, StylusType type) {
    if (type == StylusType.unknown || type == StylusType.finger) {
      return Theme.of(context).disabledColor;
    }
    return Theme.of(context).colorScheme.primary;
  }

  static String _labelForType(StylusType type) {
    switch (type) {
      case StylusType.applePencil:
        return 'Apple Pencil (1st gen)';
      ...
      case StylusType.wacomEmr:
        return 'Wacom EMR Pen';
      case StylusType.generic:
        return 'Generic Stylus';
      case StylusType.finger:
        return 'Touch Input';
      case StylusType.unknown:
        return 'No stylus detected';
    }
  }

  static String _subtitleForType(StylusType type) {
    final caps = StylusDetector.getCapabilities(type);
    if (caps.isEmpty) return 'Using finger / touch input';
    final labels = caps.map(_capLabel).join(', ');
    return labels;
  }

  static String _capLabel(StylusCapability cap) {
    switch (cap) {
      case StylusCapability.pressure:
        return 'Pressure';
      case StylusCapability.tilt:
        return 'Tilt';
      case StylusCapability.azimuth:
        return 'Azimuth';
      case StylusCapability.hover:
        return 'Hover';
      case StylusCapability.doubleTap:
        return 'Double-tap';
      case StylusCapability.squeeze:
        return 'Squeeze';
      case StylusCapability.barrelButton:
        return 'Button';
      case StylusCapability.barrelButton2:
        return 'Button 2';
      case StylusCapability.palmRejection:
        return 'Palm Rejection';
      case StylusCapability.bluetoothPairing:
        return 'BT';
      case StylusCapability.batteryLevel:
        return 'Battery';
      case StylusCapability.rollAngle:
        return 'Roll';
    }
  }
}

/// Provides capability chips for the current stylus type.
class _CapabilityChips extends StatelessWidget {
  const _CapabilityChips({required this.stylusType});

  final StylusType stylusType;

  @override
  Widget build(BuildContext context) {
    final caps = StylusDetector.getCapabilities(stylusType).toList();
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
      child: Wrap(
        spacing: 6,
...
// ─── Temporary bloc shim for stylus state ─────────────────────────────────────

/// Minimal Cubit that exposes the currently-detected [StylusType] from the
/// nearest [CanvasState].
///
/// This allows [_StylusStatusSection] to subscribe to type changes without
/// directly coupling to [CanvasBloc].
class _StylusStateBloc extends Cubit<StylusType> {
  _StylusStateBloc(super.initialState);
}

// Extend StylusSettingsPage to provide the shim bloc from canvas state.
extension _StylusPageExtension on StylusSettingsPage {
  Widget withStylusBloc(BuildContext context, Widget child) {
    // Not currently used — kept for future CanvasBloc integration.
    return child;
  }
}
